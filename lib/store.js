...
// Remove a teammate from this company. Company-scoped so an owner can only ever
// delete users within their own company.
export async function deleteUser(companyId, uid) {
  const { error } = await supa
    .from('app_users').delete()
    .eq('company_id', companyId).eq('id', uid);
  if (error) throw error;
}

// How many owners this company has — used to block deleting the last owner.
export async function countOwners(companyId) {
