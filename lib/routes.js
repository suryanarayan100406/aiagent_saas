...
api.put('/company', requireOwner, wrap(async (req, res) => {
  const allowed = (({ name, owner_name, knowledge, greeting, risky_words, hours, owner_phone, ai_enabled }) =>
    ({ name, owner_name, knowledge, greeting, risky_words, hours, owner_phone, ai_enabled }))(req.body);
  Object.keys(allowed).forEach((k) => allowed[k] === undefined && delete allowed[k]);
