// Dashboard API. Every route requires a valid Firebase token (requireAuth),
// and is scoped to the caller's company. The dashboard never touches Supabase
// directly — it goes through these endpoints, so the service key stays secret.
import express from 'express';
import admin from 'firebase-admin';
import { requireAuth, requireOwner } from './auth.js';
import { sendWhatsApp } from './whatsapp.js';
import { clearCompanyCache } from './supa.js';
import { runHealthChecks } from './health.js';
import * as store from './store.js';

export const api = express.Router();
api.use(requireAuth);

// Wrap async handlers so a rejected promise becomes a clean error response via
// the error middleware below — instead of an UnhandledPromiseRejection that
// crashes the whole Node process (Express 4 does not catch async throws).
const wrap = (fn) => (req, res, next) => Promise.resolve(fn(req, res, next)).catch(next);

// ---------- current user / company ----------
api.get('/me', wrap(async (req, res) => {
  const company = await store.getCompany(req.user.companyId);
  res.json({ user: req.user, company });
}));

// ---------- system status (owner only) ----------
// Actively probes each dependency (DB, WhatsApp, LLM, email) so the owner can
// confirm everything's wired without reading Render logs.
api.get('/health', requireOwner, wrap(async (req, res) => {
  res.json(await runHealthChecks());
}));

// ---------- contacts (conversation list) ----------
api.get('/contacts', wrap(async (req, res) => {
  res.json(await store.listContacts(req.user.companyId));
}));

api.get('/contacts/:id/messages', wrap(async (req, res) => {
  const msgs = await store.listMessages(req.user.companyId, req.params.id);
  // mark read
  await store.updateContact(req.user.companyId, req.params.id, { unread: 0 });
  res.json(msgs);
}));

api.patch('/contacts/:id', wrap(async (req, res) => {
  // update stage, notes, name, ai_enabled (human takeover)
  const allowed = (({ stage, notes, name, ai_enabled }) => ({ stage, notes, name, ai_enabled }))(req.body);
  Object.keys(allowed).forEach((k) => allowed[k] === undefined && delete allowed[k]);
  const c = await store.updateContact(req.user.companyId, req.params.id, allowed);
  res.json(c);
}));

// ---------- send a message manually (staff or owner) ----------
api.post('/contacts/:id/send', wrap(async (req, res) => {
  const { text } = req.body;
  if (!text?.trim()) return res.status(400).json({ error: 'Empty message' });
  const contacts = await store.listContacts(req.user.companyId);
  const contact = contacts.find((c) => c.id === req.params.id);
  if (!contact) return res.status(404).json({ error: 'Contact not found' });

  const ok = await sendWhatsApp(contact.wa_id, text.trim());
  if (!ok) return res.status(502).json({ error: 'WhatsApp send failed' });
  const msg = await store.addMsg(req.user.companyId, contact.id, 'owner', text.trim(), { status: 'sent' });
  await store.logEvent(req.user.companyId, 'manual_reply', { by: req.user.email });
  res.json(msg);
}));

// ---------- approvals ----------
api.get('/approvals', wrap(async (req, res) => {
  res.json(await store.listApprovals(req.user.companyId, req.query.status || 'pending'));
}));

api.post('/approvals/:id/send', wrap(async (req, res) => {
  // optionally edit the draft before sending
  const approval = await store.resolveApproval(req.user.companyId, req.params.id, 'sent');
  const text = (req.body.text || approval.draft).trim();
  const waId = approval.contacts?.wa_id;
  const ok = await sendWhatsApp(waId, text);
  if (!ok) return res.status(502).json({ error: 'WhatsApp send failed' });
  await store.addMsg(req.user.companyId, approval.contact_id, 'owner', text, { status: 'sent' });
  await store.logEvent(req.user.companyId, 'approved', { by: req.user.email });
  res.json({ ok: true });
}));

api.post('/approvals/:id/reject', wrap(async (req, res) => {
  await store.resolveApproval(req.user.companyId, req.params.id, 'rejected');
  res.json({ ok: true });
}));

// ---------- analytics ----------
api.get('/stats', wrap(async (req, res) => {
  const { events, contacts } = await store.getStats(req.user.companyId, Number(req.query.days) || 30);
  // aggregate server-side into chart-ready shape
  const byDay = {};
  const byType = {};
  const byHour = Array(24).fill(0);
  for (const e of events) {
    const d = e.created_at.slice(0, 10);
    byDay[d] = (byDay[d] || 0) + 1;
    byType[e.type] = (byType[e.type] || 0) + 1;
    byHour[new Date(e.created_at).getHours()]++;
  }
  const pipeline = {};
  for (const c of contacts) pipeline[c.stage] = (pipeline[c.stage] || 0) + 1;
  res.json({ byDay, byType, byHour, pipeline, totalContacts: contacts.length });
}));

// ---------- knowledge / config editor (owner only) ----------
api.put('/company', requireOwner, wrap(async (req, res) => {
  const allowed = (({ name, owner_name, knowledge, greeting, risky_words, hours, owner_phone, owner_email, ai_enabled }) =>
    ({ name, owner_name, knowledge, greeting, risky_words, hours, owner_phone, owner_email, ai_enabled }))(req.body);
  Object.keys(allowed).forEach((k) => allowed[k] === undefined && delete allowed[k]);
  const c = await store.updateCompany(req.user.companyId, allowed);
  clearCompanyCache(); // bot picks up new knowledge/greeting immediately
  res.json(c);
}));

// ---------- team management (owner only) ----------
api.get('/users', requireOwner, wrap(async (req, res) => {
  res.json(await store.listUsers(req.user.companyId));
}));

api.post('/users', requireOwner, wrap(async (req, res) => {
  // Create the teammate's Firebase login here (Admin SDK) and provision them into
  // this company — the owner no longer has to make the user by hand and copy a UID.
  // Back-compat: if a `uid` is passed, just link that existing user instead.
  const { uid, email, password, name, role } = req.body;
  const wantRole = role === 'owner' ? 'owner' : 'staff';

  let userUid = uid?.trim();

  if (!userUid) {
    // Create-mode: need email + password.
    if (!email?.trim() || !password) {
      return res.status(400).json({ error: 'Email and password are required.' });
    }
    if (password.length < 6) {
      return res.status(400).json({ error: 'Password must be at least 6 characters.' });
    }
    try {
      const created = await admin.auth().createUser({
        email: email.trim(),
        password,
        displayName: name?.trim() || undefined,
      });
      userUid = created.uid;
    } catch (e) {
      if (e.code === 'auth/email-already-exists') {
        // Login already exists — link it rather than failing.
        const existing = await admin.auth().getUserByEmail(email.trim());
        userUid = existing.uid;
      } else {
        throw e;
      }
    }
  }

  const u = await store.upsertUser(userUid, {
    company_id: req.user.companyId, email: email?.trim(), name: name?.trim(), role: wantRole,
  });
  res.json(u);
}));

api.delete('/users/:uid', requireOwner, wrap(async (req, res) => {
  // Remove a teammate (staff or owner) from this company and delete their login.
  const { uid } = req.params;

  // Guard: an owner can't delete their own account this way (avoids locking
  // themselves out mid-session).
  if (uid === req.user.uid) {
    return res.status(400).json({ error: "You can't remove your own account." });
  }

  // Guard: the target must belong to this company.
  const users = await store.listUsers(req.user.companyId);
  const target = users.find((u) => u.id === uid);
  if (!target) return res.status(404).json({ error: 'Teammate not found in your company.' });

  // Guard: never delete the last owner — the company must keep at least one.
  if (target.role === 'owner') {
    const owners = await store.countOwners(req.user.companyId);
    if (owners <= 1) {
      return res.status(400).json({ error: "Can't remove the only owner. Add another owner first." });
    }
  }

  await store.deleteUser(req.user.companyId, uid);

  // Best-effort: remove the Firebase login too, so the email is freed up.
  try {
    await admin.auth().deleteUser(uid);
  } catch (e) {
    console.error('Firebase deleteUser failed (row already removed):', e.code || e.message);
  }

  res.json({ ok: true });
}));

// Error middleware: any handler that throws or rejects lands here. Returns a
// clean JSON 500 instead of crashing the process. Keep last on the router.
api.use((err, req, res, _next) => {
  console.error('API error:', req.method, req.path, '-', err.code || err.message);
  if (res.headersSent) return;
  res.status(500).json({ error: err.message || 'Server error' });
});
